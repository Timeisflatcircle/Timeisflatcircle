...
def run_universe_scan(refresh=False, top=50, limit=None, output_dir="./outputs"):
    """Stage 1: automatically discover NSE small/micro-cap candidates.

    This stage intentionally uses only exchange-level market data. It does not
    invent promoter, debt, ROCE or cash-flow values; those are filled in during
    ...
    print(f"[+] Small/microcap candidates passing market/liquidity filters: {len(candidates)}")
    print(f"[+] Saved top {len(selected)} candidates to: {path}")
    print("\nTOP CANDIDATES — Stage 1 only (NOT investment recommendations)")
    print("-" * 95)
    for i, row in enumerate(selected, 1):
        print(f"{i:>2}. {row['symbol']:<15} {row['market_cap_category']:<9} MCap ₹{float(row['market_cap_cr']):>9.0f} Cr  Price ₹{float(row['price']):>8.2f}  Traded ₹{float(row['avg_daily_value_cr']):>7.2f} Cr")
    return selected
...
if __name__ == "__main__":
    ...
    cli.add_argument("--scan", action="store_true", help="Automatically discover NSE small/micro-cap candidates")
    cli.add_argument("--refresh", action="store_true", help="Refresh the NSE universe cache")
    cli.add_argument("--top", type=int, default=50, help="Number of Stage-1 candidates to save/display")
    cli.add_argument("--limit", type=int, help="Limit universe symbols for testing; omit for full NSE universe")
    cli.add_argument("--price", type=float, help="Current share price in INR for deep-analysis valuation")
